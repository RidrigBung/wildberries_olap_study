...
ch_db_name = "default"
ch_dst_table = "shk_substituition_sp_2"
...
spark_app_name = "shk_substituition_sp_2"
spark_ui_port = "8081"
...
columns_to_ch = ("shk_id", "new_shk_id", "reason_id", "wh_id")


# Схема сообщений в топике kafka. Используется при формировании batch.
schema = StructType([
    StructField("shk_id", LongType(), True),
    StructField("new_shk_id", LongType(), True),
    StructField("reason_id", LongType(), True),
...
sql_insert = f"""insert into {ch_db_name}.{ch_dst_table}
    ...
    left any join
    (
        select wh_id, office_id
        from default.dict_Warehouse
    ) t2
